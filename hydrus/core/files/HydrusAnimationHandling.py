...
def get_animation_properties(path, mime):
    ...
    if mime == HC.ANIMATION_APNG:
        ...
        ( frame_durations_ms, times_to_play ) = get_web_p_frame_durations_ms(path)
        ...
        ( frame_durations_ms, times_to_play ) = get_frame_durations_mspil_animation(path)
...
def get_apngactl_chunk_data(file_header_bytes: bytes) -> bytes | None:
...
def get_apng_duration_ms_and_num_frames(path):
    ...
    apng_actl_bytes = get_apngactl_chunk_data(file_header_bytes)
...
def get_web_p_chunks(file_bytes):
...
def get_web_p_frame_durations_ms(path):
    ...
    webp_chunks = get_web_p_chunks(file_bytes)
