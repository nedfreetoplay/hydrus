...
def GetPrettyMediaResultInfoLines( media_result: ClientMediaResult.MediaResult, only_interesting_lines = False ) -> list[ ClientMediaResultPrettyInfoObjects.PrettyMediaResultInfoLine ]:
    ...
    if CC.HYDRUS_LOCAL_FILE_STORAGE_SERVICE_KEY in current_service_keys:
        ...
        line = f'imported: {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( timestamp_ms ) )}'
        
        line_is_interesting = True
        
        tooltip = f'imported: {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( timestamp_ms ), reverse_iso_delta_setting = True )}'
    ...
    if CC.HYDRUS_LOCAL_FILE_STORAGE_SERVICE_KEY in deleted_service_keys:
        ...
        line = f'deleted from this client {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( timestamp_ms ) )} ({local_file_deletion_reason})'
        
        line_is_interesting = True
        
        tooltip = f'deleted from this client {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( timestamp_ms ), reverse_iso_delta_setting = True )} ({local_file_deletion_reason})'
    ...
    if locations_manager.IsTrashed():
        ...
        if state_trash_timestamp:
            ...
            line = f'sent to trash {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( timestamp_ms ) )}'
            tooltip = f'sent to trash {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( timestamp_ms ), reverse_iso_delta_setting = True )}'
    ...
    if file_modified_timestamp_ms is not None:
        ...
        line = f'modified: {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( file_modified_timestamp_ms ) )}'
        
        tooltip = f'modified: {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( file_modified_timestamp_ms ), reverse_iso_delta_setting = True )}'
        ...
        if timestamp_ms is not None:
            ...
            line = f'local: {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( timestamp_ms ) )}'
            
            tooltip = f'local: {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( timestamp_ms ), reverse_iso_delta_setting = True )}'
        ...
        for ( domain, timestamp_ms ) in sorted( times_manager.GetDomainModifiedTimestampsMS().items() ):
            ...
            line = f'{domain}: {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( timestamp_ms ) )}'
            
            tooltip = f'{domain}: {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( timestamp_ms ), reverse_iso_delta_setting = True )}'
    ...
    if CC.HYDRUS_LOCAL_FILE_STORAGE_SERVICE_KEY in current_service_keys and not locations_manager.inbox:
        ...
        if archived_timestamp_ms is None:
            ...
            if state_archived_timestamp:
                ...
                line = f'archived: {HydrusTime.timestamp_to_pretty_timedelta( HydrusTime.secondise_ms( archived_timestamp_ms ) )}'
