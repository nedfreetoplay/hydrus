...
class DB( HydrusDB.HydrusDB ):
    ...
    def _Backup( self, path ):
        ...
        try:
            ...
            job_status.set_status_title('backing up db')
            
            self._controller.pub( 'modal_message', job_status )
            
            job_status.set_status_text('closing db')
            ...
            for filename in self._db_filenames.values():
                ...
                job_status.set_status_text('copying ' + filename)
            ...
            additional_filenames = self._get_possible_additional_db_file_names()
            ...
            def is_cancelled_hook():
                ...
                return job_status.is_cancelled()
            ...
            job_status.set_status_text('backup complete!')
    ...
    def _CacheTagDisplaySync( self, service_key: bytes, work_period = 0.5 ):
        ...
        while len( sibling_rows_to_add ) + len( sibling_rows_to_remove ) + len( parent_rows_to_add ) + len( parent_rows_to_remove ) > 0 and not HydrusTime.time_has_passed_float( time_started + work_period ):
    ...
    def _CacheTagsSyncTags( self, tag_service_id, tag_ids, just_these_file_service_ids = None ):
        ...
        with self._make_temporary_integer_table( tag_ids, 'tag_id' ) as temp_tag_ids_table_name:
            ...
            with self._make_temporary_integer_table( unchained_tag_ids, 'tag_id' ) as temp_unchained_tag_ids_table_name:
    ...
    def _ClearOrphanFileRecords( self ):
        ...
        job_status.set_status_title('clear/fix orphan file records')
        ...
        try:
            ...
            job_status.set_status_text('looking for orphans')
            ...
            for ( umbrella_components_service_types, umbrella_master_service_id, description ) in jobs:
                ...
                job_status.set_status_text('actioning orphans')
                ...
                if len( in_components_not_in_master ) > 0:
                    ...
                    if len( those_that_exist_on_disk ) > 0:
                        ...
                        if len( import_rows ) > 0:
                            ...
                            location_context = ClientLocation.LocationContext.static_create_simple(service_key)
    ...
    def _create_db( self ):
        ...
        for ( service_key, service_type, name ) in init_service_info:
            ...
            dictionary = ClientServices.generate_default_service_dictionary(service_type)
        ...
        new_options.set_simple_downloader_formulae(ClientDefaults.get_default_simple_downloader_formulae())
        ...
        new_options.set_favourite_tag_filters(names_to_tag_filters)
        
        self.modules_serialisable.SetJSONDump( new_options )
        
        list_of_shortcuts = ClientDefaults.get_default_shortcuts()
        ...
        self._execute_many( 'INSERT INTO json_dumps_named VALUES ( ?, ?, ?, ?, ? );', ClientDefaults.get_default_script_rows())
    ...
    def _FixLogicallyInconsistentMappings( self, tag_service_key = None ):
        ...
        try:
            ...
            job_status.set_status_title('fixing logically inconsistent mappings')
    ...
    def _GetBonedStats( self, file_search_context: ClientSearchFileSearchContext.FileSearchContext = None, job_status = None ):
        ...
        with self._make_temporary_integer_table( [], 'hash_id' ) as temp_table_name:
            
            with self._make_temporary_integer_table( [], 'hash_id' ) as deleted_temp_table_name:
                ...
                if do_not_need_to_search:
                    ...
                    self._execute_many( f'INSERT OR IGNORE INTO {temp_table_name} ( hash_id ) VALUES ( ? );', ( ( hash_id, ) for hash_id in hash_ids ) )
                    
                    current_files_table_name = temp_table_name
                    
                
                hacks_going_to_work = location_context.is_one_domain()
                deleted_logic_makes_sense = location_context.includes_current() and not location_context.includes_deleted()
                ...
                if hacks_going_to_work and deleted_logic_makes_sense and current_domains_have_inverse:
                    ...
                    deleted_location_context = location_context.get_deleted_inverse()
                    ...
                    if do_not_need_to_search:
                        ...
                        self._execute_many( f'INSERT OR IGNORE INTO {deleted_temp_table_name} ( hash_id ) VALUES ( ? );', ( ( hash_id, ) for hash_id in hash_ids ) )
    ...
    def _GetBonedStatsFromTable(
        ...
        if earliest_import_timestamp_ms > 0:
            ...
            boned_stats[ 'earliest_import_time' ] = HydrusTime.secondise_ms( earliest_import_timestamp_ms )
        ...
        total_media_viewtime_float = HydrusTime.secondise_ms_float( canvas_types_to_total_viewtimes_ms.get( CC.CANVAS_MEDIA_VIEWER, ( 0, 0 ) )[1] + canvas_types_to_total_deleted_viewtimes_ms.get( CC.CANVAS_MEDIA_VIEWER, ( 0, 0 ) )[1] )
        ...
        total_preview_viewtime_float = HydrusTime.secondise_ms_float( canvas_types_to_total_viewtimes_ms.get( CC.CANVAS_PREVIEW, ( 0, 0 ) )[1] + canvas_types_to_total_deleted_viewtimes_ms.get( CC.CANVAS_PREVIEW, ( 0, 0 ) )[1] )
        ...
        with self._make_temporary_integer_table( useful_alternates_group_ids, 'alternates_group_id' ) as temp_alternates_group_ids_table_name:
    ...
    def _GetFileHistoryFromTable(
        ...
        if current_files_table_name == current_timestamps_ms_table_name:
            ...
            current_timestamps = [ HydrusTime.secondise_ms( timestamp_ms ) for ( timestamp_ms, ) in self._execute( f'SELECT timestamp_ms FROM {current_timestamps_ms_table_name} WHERE timestamp_ms IS NOT NULL;' ) ]
            
        else:
            
            current_timestamps = [ HydrusTime.secondise_ms( timestamp_ms ) for ( timestamp_ms, ) in self._execute( f'SELECT timestamp_ms FROM {current_files_table_name} CROSS JOIN {current_timestamps_ms_table_name} USING ( hash_id ) WHERE timestamp_ms IS NOT NULL;' ) ]
        ...
        if deleted_files_table_name == deleted_timestamps_ms_table_name:
            ...
            since_deleted_import_timestamps = [ HydrusTime.secondise_ms( timestamp_ms ) for ( timestamp_ms, ) in self._execute( f'SELECT original_timestamp_ms FROM {deleted_timestamps_ms_table_name} WHERE original_timestamp_ms IS NOT NULL;' ) ]
            
        else:
            
            since_deleted_import_timestamps = [ HydrusTime.secondise_ms( timestamp_ms ) for ( timestamp_ms, ) in self._execute( f'SELECT original_timestamp_ms FROM {deleted_files_table_name} CROSS JOIN {deleted_timestamps_ms_table_name} USING ( hash_id ) WHERE original_timestamp_ms IS NOT NULL;' ) ]
        ...
        if deleted_files_table_name == deleted_timestamps_ms_table_name:
            ...
            deleted_timestamps = [ HydrusTime.secondise_ms( timestamp_ms ) for ( timestamp_ms, ) in self._execute( f'SELECT timestamp_ms FROM {deleted_timestamps_ms_table_name} WHERE timestamp_ms IS NOT NULL ORDER BY timestamp_ms ASC;' ) ]
            ...
            deleted_timestamps = [ HydrusTime.secondise_ms( timestamp_ms ) for ( timestamp_ms, ) in self._execute( f'SELECT timestamp_ms FROM {deleted_files_table_name} CROSS JOIN {deleted_timestamps_ms_table_name} USING ( hash_id ) WHERE timestamp_ms IS NOT NULL ORDER BY timestamp_ms ASC;' ) ]
        ...
        if current_files_table_name == hydrus_local_file_storage_current_files_table_name:
            ...
            archive_timestamps = [ HydrusTime.secondise_ms( archived_timestamp_ms ) for ( archived_timestamp_ms, ) in self._execute( 'SELECT archived_timestamp_ms FROM archive_timestamps ORDER BY archived_timestamp_ms ASC;' ) ]
            ...
            # note also that we do not scrub archived time on a file delete, so this upcoming fetch is for all files ever. this is useful, so don't undo it m8
            archive_timestamps_current = [ HydrusTime.secondise_ms( archived_timestamp_ms ) for ( archived_timestamp_ms, ) in self._execute( f'SELECT archived_timestamp_ms FROM {current_files_table_name} CROSS JOIN archive_timestamps USING ( hash_id );' ) ]
            archive_timestamps_deleted_both = [ ( HydrusTime.secondise_ms( archived_timestamp_ms ), HydrusTime.secondise_ms( deleted_timestamp_ms ) ) for ( archived_timestamp_ms, deleted_timestamp_ms ) in self._execute( f'SELECT archived_timestamp_ms, {deleted_timestamps_ms_table_name}.timestamp_ms FROM {deleted_files_table_join} CROSS JOIN archive_timestamps USING ( hash_id );' ) ]
            ...
            archived_timestamps_from_delete_implication = [ HydrusTime.secondise_ms( timestamp_ms ) for ( timestamp_ms, hash_id ) in self._execute( f'SELECT timestamp_ms, hash_id as h1 FROM {deleted_files_table_join} WHERE {deleted_timestamps_ms_table_name}.timestamp_ms IS NOT NULL AND NOT EXISTS ( SELECT 1 FROM archive_timestamps WHERE hash_id = h1 );' ) ]
    ...
    def _GetFileSystemPredicates( self, file_search_context: ClientSearchFileSearchContext.FileSearchContext, force_system_everything = False ):
        ...
        if location_context.is_all_known_files():
            ...
            if tag_service_key == CC.COMBINED_TAG_SERVICE_KEY:
                ...
                if force_system_everything or self._controller.new_options.get_boolean('show_system_everything'):
    ...
    def _GetOptions( self ):
        ...
        if result is None:
            ...
            options = ClientDefaults.get_client_default_options()
            ...
            default_options = ClientDefaults.get_client_default_options()
    ...
    def _get_possible_additional_db_file_names( self ):
        ...
        paths = HydrusDB.HydrusDB._get_possible_additional_db_file_names( self )
    ...
    def _GetRelatedTagCountsForOneTag( self, tag_display_type, file_service_id, tag_service_id, search_tag_id, max_num_files_to_search, stop_time_for_finding_results = None ) -> tuple[ collections.Counter, bool ]:
        ...
        if stop_time_for_finding_results is not None:
            ...
            def cancelled_hook():
                ...
                return HydrusTime.time_has_passed_precise( stop_time_for_finding_results )
        ...
        for mappings_table_name in mappings_table_names:
            ...
            loop_of_results = self._sti( self._execute_cancellable( query, (), cancelled_hook ) )
    ...
    def _GetRelatedTags( self, file_service_key, tag_service_key, search_tags, max_time_to_take = 0.5, max_results = 100, concurrence_threshold = 0.04, search_tag_slices_weight_dict = None, result_tag_slices_weight_dict = None, other_tags_to_exclude = None ):
        ...
        stop_time_for_finding_results = HydrusTime.get_now_precise() + ( max_time_to_take * 0.85 )
        ...
        with self._make_temporary_integer_table( search_tag_ids_to_search_tags.keys(), 'tag_id' ) as temp_tag_id_table_name:
        ...
        with self._make_temporary_integer_table( all_tag_ids, 'tag_id' ) as temp_tag_id_table_name:
    ...
    def _GetTablesAndColumnsUsingDefinitions( self, content_type ):
        ...
        return HydrusLists.mass_extend( ( module.GetTablesAndColumnsThatUseDefinitions( content_type ) for module in self._modules ) )
    ...
    def _GetTrashHashes( self, limit = None, minimum_age = None ):
        ...
        if minimum_age is None:
            ...
            timestamp_cutoff = HydrusTime.get_now() - minimum_age
        ...
        if HG.db_report_mode:
            ...
            if minimum_age is not None:
                ...
                message += ' with minimum age ' + HydrusTime.timestamp_to_pretty_timedelta( timestamp_cutoff, just_now_threshold = 0 ) + ','
    ...
    def _ImportFile( self, file_import_job: ClientImportFiles.FileImportJob ):
        ...
        if not file_import_status.AlreadyInDB():
            ...
            timestamp_data = ClientTime.TimestampData.static_file_modified_time(file_modified_timestamp_ms)
    ...
    def _MigrationGetMappings( self, database_temp_job_name, location_context: ClientLocation.LocationContext, tag_service_key, hash_type, tag_filter, content_statuses ):
        ...
        while not we_should_stop:
            ...
            we_should_stop = len( data ) >= 256 or ( len( data ) > 0 and HydrusTime.time_has_passed_precise( time_started_precise + 1.0 ) )
    ...
    def _MigrationStartMappingsJob(
        ...
        if hashes is not None:
            ...
            with self._make_temporary_integer_table( [], 'tag_id' ) as temp_tag_ids_table_name:
    ...
    def _PerceptualHashesSearchForPotentialDuplicates( self, search_distance: int, work_period: float | None = None ):
        ...
        time_started_float = HydrusTime.get_now_float()
        ...
        while len( group_of_hash_ids ) > 0:
            ...
            for ( i, hash_id ) in enumerate( group_of_hash_ids ):
                ...
                if work_period is not None and HydrusTime.time_has_passed_float( time_started_float + work_period ):
    ...
    def _RegenerateLocalHashCache( self ):
        ...
        try:
            ...
            job_status.set_status_title('resynchronising local hashes cache')
    ...
    def _RegenerateLocalTagCache( self ):
        ...
        try:
            ...
            job_status.set_status_title('regenerating local tag cache')
    ...
    def _RegenerateTagCacheSearchableSubtagMaps( self, tag_service_key = None ):
        ...
        try:
            ...
            job_status.set_status_title('regenerate tag fast search cache searchable subtag map')
            ...
            def status_hook( s ):
                ...
                job_status.set_status_text(s, 2)
    ...
    def _RegenerateTagCache( self, tag_service_key = None ):
        ...
        try:
            ...
            job_status.set_status_title('regenerating tag fast search cache')
    ...
    def _RegenerateTagDisplayMappingsCache( self, tag_service_key = None ):
        ...
        try:
            ...
            job_status.set_status_title('regenerating tag display mappings cache')
    ...
    def _RegenerateTagDisplayPendingMappingsCache( self, tag_service_key = None ):
        ...
        try:
            ...
            job_status.set_status_title('regenerating tag display pending mappings cache')
    ...
    def _RegenerateTagMappingsCache( self, tag_service_key = None ):
        ...
        try:
            ...
            job_status.set_status_title('regenerating tag mappings cache')
    ...
    def _RegenerateTagMappingsTags( self, tags, tag_service_key = None ):
        ...
        try:
            ...
            job_status.set_status_title('regenerating tag mappings')
    ...
    def _RegenerateTagPendingMappingsCache( self, tag_service_key = None ):
        ...
        try:
            ...
            job_status.set_status_title('regenerating tag pending mappings cache')
    ...
    def _RepairInvalidTags( self, job_status: ClientThreading.JobStatus | None = None ):
        ...
        if job_status is not None:
            ...
            if not job_status.is_cancelled():
    ...
    def _RepopulateMappingsFromCache( self, tag_service_key = None, job_status = None ):
        ...
        for tag_service_id in tag_service_ids:
            ...
            for ( group_of_hash_ids, num_done, num_to_do ) in HydrusDB.read_large_id_query_in_separate_chunks( self._c, select_statement, BLOCK_SIZE ):
                ...
                with self._make_temporary_integer_table( group_of_hash_ids, 'hash_id' ) as temp_table_name:
                    ...
                    num_rows_recovered += self._get_row_count()
        ...
        if job_status is not None:
            ...
            job_status.set_status_text('Done! Rows recovered: {}'.format(HydrusNumbers.to_human_int(num_rows_recovered)))
    ...
    def _RepopulateTagCacheMissingSubtags( self, tag_service_key = None ):
        ...
        try:
            ...
            job_status.set_status_title('repopulate tag fast search cache subtags')
    ...
    def _RepopulateTagDisplayMappingsCache( self, tag_service_key = None ):
        ...
        try:
            ...
            job_status.set_status_title('repopulating tag display mappings cache')
            ...
            for ( i, file_service_id ) in enumerate( file_service_ids ):
                ...
                for ( group_of_ids, num_done, num_to_do ) in HydrusDB.read_large_id_query_in_separate_chunks( self._c, 'SELECT hash_id FROM {};'.format( table_name ), 1024 ):
                    ...
                    with self._make_temporary_integer_table( group_of_ids, 'hash_id' ) as temp_hash_id_table_name:
    ...
    def _ResetRepository( self, service ):
        ...
        try:
            ...
            job_status.set_status_text(prefix + ': deleting service')
            
            self._controller.pub( 'modal_message', job_status )
            
            self._DeleteService( service_id )
            
            job_status.set_status_text(prefix + ': recreating service')
            ...
            job_status.set_status_text(prefix + ': done!')
    ...
    def _ResetRepositoryProcessing( self, service_key: bytes, content_types ):
        ...
        try:
            ...
            job_status.set_status_text('{}: calculating'.format(prefix))
            ...
            job_status.set_status_text('{}: recalculating'.format(prefix))
            ...
            self._execute_many( 'DELETE FROM service_info WHERE service_id = ? AND info_type = ?;', ( ( service_id, info_type ) for info_type in service_info_types_to_delete ) )
    ...
    def _ResyncTagMappingsCacheFiles( self, tag_service_key = None ):
        ...
        try:
            ...
            job_status.set_status_title('resyncing tag mappings cache files')
            ...
            for file_service_id in file_service_ids:
                ...
                for tag_service_id in tag_service_ids:
                    ...
                    with self._make_temporary_integer_table( hash_ids_in_file_service_and_not_in_cache, 'hash_id' ) as temp_hash_id_table_name:
    ...
    def _update_db( self, version ):
        ...
        if version == 592:
            ...
            try:
                ...
                def correct_permits_everything_test( p: ClientAPI.APIPermissions ):
                    ...
                    basic_permissions = p.get_basic_permissions()
                ...
                def ask_what_to_do_permissions_stuff_593():
                    ...
                    result = ClientGUIDialogsQuick.GetYesNo(CG.client_controller.get_main_tlw(), message, title ='Re-do permissions update?', yes_label ='yes, re-run the update', no_label ='no, I checked everything in "review services" already')
                ...
                if len( underpermitted_yet_set_to_permits_everything ) > 0:
                    ...
                    do_permissions_stuff = self._controller.call_blocking_to_qt_tlw(ask_what_to_do_permissions_stuff_593)
        ...
        if version == 594:
            ...
            try:
                ...
                db_location_context = self.modules_files_storage.GetDBLocationContext(ClientLocation.LocationContext.static_create_simple(CC.HYDRUS_LOCAL_FILE_STORAGE_SERVICE_KEY))
                ...
                if len( false_positive_hash_ids ) > 0:
                    ...
                    def ask_what_to_do_false_positive_modified_dates():
                        ...
                        result = ClientGUIDialogsQuick.GetYesNo(CG.client_controller.get_main_tlw(), message, title ='Reset modified dates?', yes_label ='do it', no_label ='do not do it')
                        
                        return result == QW.QDialog.DialogCode.Accepted
                        
                    
                    do_it = self._controller.call_blocking_to_qt_tlw(ask_what_to_do_false_positive_modified_dates)
        ...
        if version == 606:
            ...
            if do_it:
                ...
                try:
                    ...
                    self._create_index( 'main.file_viewing_stats', [ 'viewtime_ms' ] )
            ...
            try:
                ...
                new_options.set_boolean('show_extended_single_file_info_in_status_bar', True)
                ...
                for option_name in [
                    ...
                    the_time = new_options.get_noneable_integer(option_name)
                    ...
                    new_options.set_noneable_integer(option_name + '_ms', the_time_ms)
        ...
        if version == 609:
            ...
            try:
                ...
                new_options.set_integer('file_viewing_stats_menu_display', CC.FILE_VIEWING_STATS_MENU_DISPLAY_SUMMED_AND_THEN_SUBMENU)
        ...
        if version == 611:
            ...
            if not self._table_exists( 'main.duplicate_files_auto_resolution_rules' ):
            ...
            if not self._table_exists( 'main.duplicates_files_auto_resolution_rule_count_cache' ):
        ...
        if version == 615:
            ...
            if self._table_exists( 'main.duplicate_files_auto_resolution_rules' ):
                ...
                if do_it:
                    ...
                    if found_some:
                        ...
                        def notify_deleting_auto_resolution_rules():
                            ...
                            ClientGUIDialogsMessage.ShowInformation(CG.client_controller.get_main_tlw(), message)
                            
                        
                        self._controller.call_blocking_to_qt_tlw(notify_deleting_auto_resolution_rules)
            ...
            try:
                ...
                if not user_wants_us_to_leave_it_on:
                    ...
                    if results.get( HC.SERVICE_INFO_NUM_FILES, 0 ) > 10000:
                        ...
                        new_options.set_boolean('show_system_everything', False)
        ...
        if version == 618:
            ...
            try:
                ...
                current_value = new_options.get_integer('ms_to_wait_between_physical_file_deletes')
                
                if current_value == 250:
                    
                    new_options.set_integer('ms_to_wait_between_physical_file_deletes', 600)
            ...
            try:
                ...
                jobs = [
                    (
                        ...
                        CAC.ApplicationCommand.static_create_simple_command( CAC.SIMPLE_COPY_FILES ),
        ...
        if version == 625:
            ...
            try:
                ...
                new_options.set_boolean('allow_remove_on_manage_tags_input', False)
        ...
        if version == 627:
            ...
            try:
                ...
                draw_thumbnail_rating_icon_size_px = new_options.get_float('draw_thumbnail_rating_icon_size_px')
                media_viewer_rating_icon_size_px = new_options.get_float('media_viewer_rating_icon_size_px')
                
                new_options.set_float('thumbnail_rating_incdec_width_px', draw_thumbnail_rating_icon_size_px * 2)
                new_options.set_float('media_viewer_rating_incdec_width_px', media_viewer_rating_icon_size_px * 2)
                
                new_options.set_float('preview_window_rating_icon_size_px', media_viewer_rating_icon_size_px)
                new_options.set_float('preview_window_rating_incdec_width_px', media_viewer_rating_icon_size_px * 2)
                new_options.set_float('dialog_rating_icon_size_px', draw_thumbnail_rating_icon_size_px)
                new_options.set_float('dialog_rating_incdec_width_px', draw_thumbnail_rating_icon_size_px * 2)
                
                new_options.set_boolean('remove_leading_url_double_slashes', False)
        ...
        if version == 631:
            ...
            try:
                ...
                new_options.set_boolean('preview_window_hover_top_right_shows_popup', True)
                new_options.set_boolean('draw_top_right_hover_in_preview_window_background', True)
                new_options.set_boolean('replace_percent_twenty_with_space_in_gug_input', True)
        ...
        if version == 633:
            ...
            try:
                ...
                thumbnail_rating_incdec_width_px = new_options.get_float('thumbnail_rating_incdec_width_px')
                media_viewer_rating_incdec_width_px = new_options.get_float('media_viewer_rating_incdec_width_px')
                preview_window_rating_incdec_width_px = new_options.get_float('preview_window_rating_incdec_width_px')
                dialog_rating_incdec_width_px = new_options.get_float('dialog_rating_incdec_width_px')
                
                new_options.set_float('thumbnail_rating_incdec_height_px', thumbnail_rating_incdec_width_px / 2)
                new_options.set_float('media_viewer_rating_incdec_height_px', media_viewer_rating_incdec_width_px / 2)
                new_options.set_float('preview_window_rating_incdec_height_px', preview_window_rating_incdec_width_px / 2)
                new_options.set_float('dialog_rating_incdec_height_px', dialog_rating_incdec_width_px / 2)
            ...
            try:
                ...
                for rule_id in rule_ids:
                    ...
                    if self._table_exists( original_table_name ):
                        ...
                        self._create_index( new_table_name, [ 'larger_media_id', 'smaller_media_id' ], unique = True )
                        self._create_index( new_table_name, [ 'timestamp_ms' ] )
                        ...
                        self._execute_many( f'INSERT OR IGNORE INTO {new_table_name} ( smaller_media_id, larger_media_id, timestamp_ms ) VALUES ( ?, ?, ? );', inserts )
        ...
        if version == 638:
            ...
            if not self._table_exists( 'main.shape_search_cache_numbers' ):
        ...
        if version == 642:
            ...
            try:
                ...
                new_options.set_boolean('use_legacy_mpv_mediator', False)
                
                #
                
                media_view_options = new_options.get_media_view_options()
                ...
                new_options.set_media_view_options(media_view_options)
        ...
        if version == 643:
            ...
            def ask_what_to_do_transparency_recheck_644( num_transparent_files ):
                ...
                result = ClientGUIDialogsQuick.GetYesNo(CG.client_controller.get_main_tlw(), message, title ='Re-do transparency check?', yes_label ='yes, re-scan these files', no_label ='no, do not do it')
            ...
            try:
                ...
                with self._make_temporary_integer_table( all_local_hash_ids, 'hash_id' ) as temp_hash_ids_table_name:
                    ...
                    if len( hash_ids ) > 0:
                        ...
                        do_transparency_recheck = self._controller.call_blocking_to_qt_tlw(ask_what_to_do_transparency_recheck_644, len(hash_ids))
        ...
        if version == 648:
            ...
            try:
                ...
                if new_options.get_integer('image_cache_prefetch_limit_percentage') < 25:
                    
                    new_options.set_integer('image_cache_prefetch_limit_percentage', 25)
        ...
        if version == 653:
            ...
            try:
                ...
                if new_options.get_integer('command_palette_num_chars_for_results_threshold') == 0:
                    
                    new_options.set_integer('command_palette_num_chars_for_results_threshold', 1)
    ...
    def _UpdateServerServices( self, admin_service_key, serverside_services, service_keys_to_access_keys, deletee_service_keys ):
        ...
        admin_credentials = admin_service.get_credentials()
    ...
    def _Vacuum( self, names: collections.abc.Collection[ str ], maintenance_mode = HC.MAINTENANCE_FORCED, stop_time = None, force_vacuum = False ):
        ...
        job_status.set_status_title('database maintenance - vacuum')
        ...
        try:
            ...
            for name in ok_names:
                ...
                try:
                    ...
                    job_status.set_status_text('vacuuming ' + name)
            ...
            job_status.set_status_text('cleaning up')
