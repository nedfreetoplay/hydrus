...
def QDateTimeToPrettyString( dt: QC.QDateTime | None, include_milliseconds = False ):
    ...
    if dt is None:
        ...
        return f'{dt.toString( phrase )} ({HydrusTime.timestamp_to_pretty_time_delta(dt.toSecsSinceEpoch(), force_no_iso = True)})'
...
class DateTimeWidgetValueRange( object ):
    ...
    def ToString( self ):
        ...
        if self._step_ms != 0:
            ...
            s += f', with step {HydrusTime.time_delta_to_pretty_time_delta(HydrusTime.secondise_ms_float(self._step_ms))}'
...
class TimeDeltaButton( QW.QPushButton ):
    ...
    def _RefreshLabel( self ):
        ...
        if value is None:
            ...
            text = HydrusTime.time_delta_to_pretty_time_delta(value)
