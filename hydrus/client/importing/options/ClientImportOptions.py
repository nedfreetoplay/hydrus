...
class CheckerOptions( HydrusSerialisable.SerialisableBase ):
    ...
    def GetPrettyCurrentVelocity( self, file_seed_cache, last_check_time, no_prefix = False ):
        ...
        if len( file_seed_cache ) == 0:
            ...
            pretty_current_velocity += HydrusNumbers.to_human_int(current_files_found) + ' files in previous ' + HydrusTime.time_delta_to_pretty_time_delta(current_time_delta)
    ...
    def GetSummary( self ):
        ...
        if self._never_faster_than == self._never_slower_than:
            ...
            timing_statement = 'Checking every ' + HydrusTime.time_delta_to_pretty_time_delta(self._never_faster_than) + '.'
            ...
            timing_statement = f'Trying to get {intended_files_per_check_str} files per check, never faster than ' + HydrusTime.time_delta_to_pretty_time_delta(self._never_faster_than) + ' and never slower than ' + HydrusTime.time_delta_to_pretty_time_delta(self._never_slower_than) + '.'
        ...
        if death_files_found == 0:
            ...
            death_statement = 'Stopping if file velocity falls below ' + HydrusNumbers.to_human_int(death_files_found) + ' files per ' + HydrusTime.time_delta_to_pretty_time_delta(death_time_delta) + '.'
